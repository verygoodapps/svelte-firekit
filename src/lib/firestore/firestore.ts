...
import { authUser } from "$lib/auth/user.svelte.js";
...
export async function getDocumentOrError<T>(
  ref: DocPredicate<T>
): Promise<{ data?: T; ref?: DocumentReference<T>; error?: FirestoreError }> {
  try {
    const _ref = docRef<T>(ref);
    const docSnap = await getDoc(_ref);
    return {
      data: docSnap.exists()
        ? { ...(docSnap.data() as T), id: docSnap.id }
        : null,
      ref: _ref,
    };
  } catch (error) {
